...
collectResultsCrowdFlower <- function(job_id, title = "Task", unzip = T, download = T, sep = ","){
	...
	csv_filename <- paste(DATASET_FOLDER,"/f",job_id,".csv", sep="")
	if (unzip){
		csv_filename <-unz(destfile, csv_filename)
	}
	data <- read.table(csv_filename, header=T, sep=sep, quote = "\"",comment.char = "")
